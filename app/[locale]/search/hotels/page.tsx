        ...
        <div className="container mx-auto px-4 py-16">
          <Card className="max-w-2xl mx-auto rounded-xl shadow-lg border-0">
            <CardContent className="p-8 text-center">
              ...
              <Link href={`/${locale}`}>
                <Button className="bg-brand-secondary hover:bg-brand-accent text-white">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <input 
                      ...
                      min="0" 
                      max="2000" 
                      className="w-full h-2 bg-gradient-to-r from-brand-primary/20 to-brand-secondary/20 rounded-lg appearance-none cursor-pointer accent-brand-primary" 
                    />
                    ...
                    <div className="space-y-3">
                      {[5, 4, 3].map(star => (
                        <label key={star} className="flex items-center gap-3 cursor-pointer group">
                          ...
                          <span className="text-sm group-hover:text-brand-primary transition">{star} {locale === 'ar' ? 'نجوم' : 'Stars'}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                      ...
                      {[
                        ...
                        <label key={i} className="flex items-center gap-3 cursor-pointer group">
                          <input 
                            ...
                            className="w-4 h-4 rounded border-gray-300 text-brand-primary focus:ring-brand-primary cursor-pointer" 
                          />
                          <amenity.icon className="w-4 h-4 text-brand-primary" />
                          <span className="text-sm group-hover:text-brand-primary transition">{amenity.label}</span>
                        </label>
                      ))}
                    </div>
                  </div>
            ...
            <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <p className="text-sm text-gray-600">
                  {locale === 'ar' ? 'إظهار' : 'Showing'} <span className="font-semibold text-brand-primary">{hotels.length}</span> {locale === 'ar' ? 'نتيجة' : 'results'}
                </p>
              </div>
              
              <div className="flex gap-3">
                <Button variant="outline" className="rounded-lg border-brand-primary/30 hover:bg-brand-primary/10 transition">
                  <Map className="w-4 h-4 mr-2" />
                  {locale === 'ar' ? 'عرض الخريطة' : 'Map View'}
                </Button>
                
                <div className="relative">
                  <select className="px-4 py-2 pr-10 border border-gray-300 rounded-lg bg-white hover:border-brand-primary focus:ring-2 focus:ring-brand-primary focus:border-transparent transition appearance-none cursor-pointer">
                ...
                <a key={hotel.id} href={`/${locale}/hotel/${hotel.id}`}>
                  <Card className="group overflow-hidden rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 border-0">
                    <div className="flex flex-col md:flex-row gap-0 md:gap-6">
                      <div className="relative w-full md:w-80 h-64 md:h-auto bg-gradient-to-br from-brand-primary via-brand-primary-light to-brand-secondary overflow-hidden flex-shrink-0">
                      ...
                      <div className="flex-1 p-6">
                        <div className="flex flex-col h-full justify-between">
                          <div>
                            <h3 className="text-2xl font-bold mb-2 group-hover:text-brand-primary transition">
                              {hotel.name}
                            </h3>
                            ...
                            <div className="flex gap-2 flex-wrap mb-4">
                              ...
                              <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-brand-primary/10 text-brand-primary text-xs font-medium rounded-full">
                          ...
                          <div className="flex items-end justify-between pt-4 border-t">
                            <div>
                              ...
                              <div className="flex items-baseline gap-2">
                                <span className="text-3xl font-bold bg-gradient-to-r from-brand-primary to-brand-primary-light bg-clip-text text-transparent">
                                  {hotel.minPrice}
                                </span>
                            ...
                            <Button className="bg-brand-secondary hover:bg-brand-accent text-white rounded-lg shadow-lg hover:shadow-xl transition-all">
